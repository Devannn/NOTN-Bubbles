...
const BubbleStats = () => {
        ...
        try {
            ...
            if (data && data.rsp && data.rsp.data) {
                const hosts = data.rsp.data
                    .filter(host => host.ip.startsWith('192.168.1.'))
                    .map(host => host.ip);
                console.log('Filtered 192.168.1.x hosts:', hosts);
    ...
    const fetchHostStats = async (hostIP) => {
        try {
            ...
            console.log(`Transformed stats for ${hostIP}:`, transformedData);
